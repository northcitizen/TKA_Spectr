...
extern uint8_t Ra, Rall, R9, Ri[14], old_meas_type_E,old_meas_type_L, SD_Detect;

extern uint8_t SD_Detect, write_FileNum;
extern float E_day, E_day_Wt, E_Night, SP_Measure=0, PPFD_PPL_Measure, PPFD_PPL_Blue_Measure, PPFD_PPL_Green_Measure, PPFD_PPL_Red_Measure, PPFD_PPL_Far_Red_Measure,
	ELr_Measure, ELb_Measure;
int16_t delta_Eab_Measure;
extern uint16_t	Tc_Measure, lambda_d_Measure, lambda_c_Measure, Calibration_date=1, Calibration_month=1, Calibration_year=2020;
...
extern uint16_t colorimetry_XYZ[3], cnt_delay_bar, Serial_part_device=0, Serial_number_device=0;
extern int16_t colorimetry_LAB[3];
extern float colorimetry_xy[2], colorimetry_uv[2];
...
uint8_t Language_status = Ru, Graph_Field = 0x00, Color_Field = 0x00, Measure_Color_xy = 0x00, Color_rend_Field = 0x00, Bluetooth, Source_Type, Prev_Inf_Screen,Prev_Inf_Screen2, Rotation_Screen = 0x00,
...
extern double percentage_charge=0, bar_CQS=0, bar_CRI=0, SDWr_Status_bar=0;
...
{
	TFT_FillScreen_DMA(TFT_Black_Bkgr);
	GUI_TextRu_Wait(10, 50);
}

void GUI_Wait_END_Screen(void)
{
	TFT_FillScreen_DMA(TFT_Black_Bkgr);
	GUI_TextRu_Wait_END(10, 50);
}
...
void GUI_ColorRend_Screen(){
	...
	if (Color_rend_Field){
//		CRI_Draw(Rotation_Screen_Rend, data, 15, 20, 20);
	}
	else{
//		CQS_Draw(Rotation_Screen_Rend, Q_i, Qa, Qp, Qf);
	...
	if(Calc_ColorRend != Calc_ColorRend_old){
		...
		if (Color_rend_Field){
//			CRI_Draw(Rotation_Screen_Rend, data,  15, 20, 20);//CRI_Draw(Rotation_Screen_Rend, Ri, Ra, Rall, R9);
		}
		else{
//			CQS_Draw(Rotation_Screen_Rend, Q_i, Qa, Qp, Qf);
		}
