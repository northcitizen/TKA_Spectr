...
volatile uint8_t colorRendCRI;
volatile float progress_bar;
...
uint8_t Language_status = Ru, Graph_Field = 0x00, Color_Field = 0x00, preColor_Field = 0x00, Measure_Color_xy = 0x00, Bluetooth, Source_Type, Prev_Inf_Screen,Prev_Inf_Screen2, Rotation_Screen = 0x00,
Calc_ColorRend = 0x00, Calc_ColorRend_old = 0x00, current_state_Measure_Draw = 0;
volatile uint8_t Color_rend_Field = 0x00;
 ...
 old_meas_type_L = 2;
	if(preGUI_screen_state != GUI_screen_state || Language_status_prev != Language_status){
		...
		if(Language_status == En){
			...
			GUI_Text_CRI(30, 325);
				GUI_RadioBox(185, 318, !colorRendCRI, 0x01);
				GUI_RadioBox(185, 362, colorRendCRI, 0x01);
...
colorimetry_xy_buff[1] = (Measure_Color_xy == 0x00) ? colorimetry_xy1964[1] : colorimetry_xy1931[1];
	...
	if(Color_Field&Color_CIE_xy)//xy
	{
		...
		flag = 1;
		 ...
		 if(lowSignal || highSignal)
		 			GUI_Text_xy_Measure(20, 70, colorimetry_xy_buff[0], colorimetry_xy_buff[1], 1, 0);
	} else
	if(Color_Field&Color_CIE_Luv) //Luv
	{
		...
		if(lowSignal || highSignal)
			GUI_Text_uv_Measure(20, 70, 0.0, 0.0, 1, 0);
	...
	}else if(Color_Field&Color_CIE_Lab)
	{
		...
		if(lowSignal || highSignal)
					GUI_Text_LAB_Measure(20, 54, colorimetry_LAB[0], colorimetry_LAB[1], colorimetry_LAB[2], 1, 0);
	}
	TFT_direction = 0x00;
	GUI_SignalLevel();
...
void GUI_ColorRend_Screen(){
 ...
 if(lowSignal || highSignal || CCT_NO_FLAG){
	Ra = 0;
	Qa = 0;
	Qp=0;
	Qf = 0;
	...
	if (/*Color_rend_Field&CRI_CQS*/colorRendCRI){
	...
	if(CCT_NO_FLAG || lowSignal || highSignal)
	{
		Ra = 0;
			Qa = 0;
			Qp=0;
...
void measure()
{
	...
	while (start) {
		cnt_delay++;
		GUI_SignalLevel();
		if ((cnt_delay > 20 && exp_num < 6) || (cnt_delay > 50 && (exp_num >= 6 && exp_num < 8)) || (cnt_delay > 100 && exp_num >= 8)) {
			auto_exposure();
			max_el = 0;
			cnt_delay = 0;
...
case Measure3_Screen:
		...
		case Graph_Screen:
			...
			Calculate_uv((Measure_Color_xy == 0x00) ? colorimetry_xy1964 : colorimetry_xy1931);
...
//					if(Rotation_Screen_Color < 0x02) {Rotation_Screen_Color++;} else{Rotation_Screen_Color = 0x00;}
		...
		case Color_Rendition_Screen:
			...
			if (Measure_Field & PPFD_BGR) {
				PPFD_PPL_Blue_Measure = (Calculate_PPFD_PPL_Range(Line_Rabs_buff, WaveLenght, BLUE_RANGE)); //*100000
				PPFD_PPL_Green_Measure = (Calculate_PPFD_PPL_Range(Line_Rabs_buff, WaveLenght, GREEN_RANGE)); //*100000
				PPFD_PPL_Red_Measure = (Calculate_PPFD_PPL_Range(Line_Rabs_buff,WaveLenght, RED_RANGE)); //*100000
				PPFD_PPL_Far_Red_Measure = (Calculate_PPFD_PPL_Range(Line_Rabs_buff, WaveLenght, FAR_RED_RANGE)); //*100000
			}
		...
		case ColorRendSet_Screen:
				...
				{	
					...
					Calculate_uv1976((Measure_Color_xy == 0x00) ? colorimetry_xy1964 : colorimetry_xy1931);Calculate_uv((Measure_Color_xy == 0x00) ? colorimetry_xy1964 : colorimetry_xy1931);
					Calculate_Lab((Measure_Color_xy == 0x00) ? colorimetry_XYZ1964 : colorimetry_XYZ1931, Measure_Color_xy, Source_Type);
					Calculate_Lambda_Dominant(Line_Rabs_buff, Measure_Color_xy);
					Calculate_Lambda_Dominant(Line_Rabs_buff, Measure_Color_xy);
					ELr_Measure = Calculate_ELr(Line_Rabs_buff,Hazard_Retina);
