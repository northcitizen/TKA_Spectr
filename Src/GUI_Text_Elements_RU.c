...
void GUI_TextRu_Wait(uint16_t X, uint16_t Y)
{
	...
	TFT_SetFont(&Font16RU_arch_small);
	X+=16; Y+=5;
	TFT_DrawCharRus(X, Y, 'д');
	X+=13;
	TFT_DrawCharRus(X, Y, 'е');
	X+=11;
	TFT_DrawCharRus(X, Y, 'т');

	X+=23;
	TFT_DrawCharRus(X, Y, 'з');
	X+=13;
	TFT_DrawCharRus(X, Y, 'а');
	X+=13;
	TFT_DrawCharRus(X, Y, 'г');
	X+=12;
	TFT_DrawCharRus(X, Y, 'р');
	X+=12;
	TFT_DrawCharRus(X, Y, 'у');
	X+=13;
	TFT_DrawCharRus(X, Y, 'з');
	X+=13;
	TFT_DrawCharRus(X, Y, 'к');
	...
	TFT_DrawCharRus(X, Y, 'а');
	X+=13;
	TFT_DrawCharRus(X, Y, 'н');
	X+=13;
	TFT_DrawCharRus(X, Y, 'н');
	X+=14;
	TFT_DrawCharRus(X, Y, 'ы');
	X+=13;
	TFT_DrawCharRus(X, Y, 'х');

	X=X0; Y+=35;
	TFT_SetFont(&Font16RU_arch_big);
	TFT_DrawCharRus(X, Y, 'П');
	TFT_SetFont(&Font16RU_arch_small);
	X+=16; Y+=5;
	TFT_DrawCharRus(X, Y, 'о');
	X+=13;
	TFT_DrawCharRus(X, Y, 'ж');
	X+=13;
	TFT_DrawCharRus(X, Y, 'а');
	X+=12;
...
void GUI_TextRu_Wait_END(uint16_t X, uint16_t Y)
{
	...
	TFT_DrawCharRus(X, Y, 'а');
	X+=13;
	TFT_DrawCharRus(X, Y, 'в');
	X+=13;
	TFT_DrawCharRus(X, Y, 'е');
	X+=12;
	TFT_DrawCharRus(X, Y, 'р');
	X+=14;
	TFT_DrawCharRus(X, Y, 'ш');
	X+=14;
	TFT_DrawCharRus(X, Y, 'е');
	X+=12;
	TFT_DrawCharRus(X, Y, 'н');
